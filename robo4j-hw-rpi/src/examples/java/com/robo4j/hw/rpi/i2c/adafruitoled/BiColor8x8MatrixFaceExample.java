...
public class BiColor8x8MatrixFaceExample {
	...
	public static void main(String[] args) throws Exception {
		...
		for (char[] face : availableFaces) {
			matrix.clear();
			matrix.display();
			byte[] faceByte = LEDBackpackUtils.convertBiColorMatrixCharSequenceToArray(matrix.getMatrixSize(), ',',
					face);
			PackElement[] faceElements = LEDBackpackUtils.createByArray(matrix.getMatrixSize(), faceByte);
			matrix.addPixes(faceElements);
			matrix.display();
