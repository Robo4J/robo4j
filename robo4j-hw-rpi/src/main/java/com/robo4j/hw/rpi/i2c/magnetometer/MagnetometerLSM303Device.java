...
public class MagnetometerLSM303Device extends AbstractI2CDevice implements ReadableDevice<Tuple3f> {
    ...
    public MagnetometerLSM303Device(I2cBus bus, int address, Mode mode, Rate rate, boolean enableTemp) throws IOException {
        this(bus, address, mode, rate, enableTemp, new Tuple3f(0, 0, 0), Matrix3f.createIdentity());
    }

    public MagnetometerLSM303Device(I2cBus bus, int address, Mode mode, Rate rate, boolean enableTemp, Tuple3f bias,
...
//		int n = i2CConfig.read(OUT_X_H_M, data, 0, RESULT_BUFFER_SIZE);
     ...
     *
     * @param magResult a result to use to calculate the compass heading.
