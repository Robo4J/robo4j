...
class HearbeatMessageCodec {
    private static final Logger LOGGER = LoggerFactory.getLogger(HearbeatMessageCodec.class);
    private static final byte PROTOCOL_VERSION = 0;
    private static final int PROTOCOL_VERSION_BYTE_LENGTH = 1;
    private static final int PACKAGE_LENGTH_BYTE_LENGTH = 4;
    private static final int ID_LENGTH_BYTE_LENGTH = 2;
    private static final int HEART_BEAT_PERIOD_BYTE_LENGTH = 4;
    private static final int MAX_U2 = 65535;
    private static final int MAX_U1 = 255;
    private static final byte[] MAGIC = new byte[]{(byte) 0xC0, (byte) 0xFF};
    ...
    private static String crop(int maxLength, String string, String entityName) {
        if (string.getBytes().length > maxLength) {
            LOGGER.error("The string for {} was too long for the protocol (max = {}) and has been cropped. The value started with {}", entityName, maxLength, string.substring(0, 20));
