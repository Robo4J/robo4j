...
/**
 * This is the default implementation for a local {@link RoboContext}. Contains RoboUnits, a lookup
 * service for references to RoboUnits, and a system level life cycle.
 *
...
final class RoboSystem implements RoboContext {
	...
	RoboSystem(String uid, Configuration configuration) {
		...
		var workThreadFactory = new RoboThreadFactory.Builder(THREAD_GROUP_NAME_WORKER_POOL).addThreadPrefix(THREAD_GROUP_NAME_WORKER_POOL).build();

		var blockingThreadFactory = new RoboThreadFactory.Builder(THREAD_GROUP_BLOCKING_NAME_BLOCKING_POOL)
				.addThreadPrefix(THREAD_GROUP_BLOCKING_NAME_BLOCKING_POOL).build();
	...
	public void start() {
		...
		if (currentState == LifecycleState.INITIALIZED || currentState == LifecycleState.STOPPED) {
		...
		final URI listeningURI = messageServer != null ? getListeningURI(messageServer) : null;
		final ContextEmitter emitter = initEmitter(emitterConfiguration, listeningURI);
		if (emitter != null) {
	...
	@Override
	public void stop() {
		// NOTE(Marcus/Sep 4, 2017): We may want schedule the stops in the future.
	...
	private static void shutdownUnit(RoboUnit<?> unit) {
		// NOTE(Marcus/Aug 11, 2017): Should really be scheduled and done in parallel.
		unit.shutdown();
	...
	private ContextEmitter initEmitter(Configuration emitterConfiguration, URI uri) {
		if (messageServer != null && uri != null) {
			...
			// Check if standalone mode is intended (explicit or implicit)
			if (isStandaloneModeIntended(configuration)) {
				LOGGER.debug("Operating in standalone mode - networking disabled.");
			} else {
				// Discovery configured but no server - likely misconfiguration
				LOGGER.warn("Context emitter configured but no message server available. Server was: " + messageServer + " uri was: " + uri);
			}
		}
		return null;
	}
	...
	private RoboContextDescriptor createDescriptor(URI uri, Configuration emitterConfiguration) {
		int heartbeatInterval = emitterConfiguration.getInteger(ContextEmitter.KEY_HEARTBEAT_INTERVAL, ContextEmitter.DEFAULT_HEARTBEAT_INTERVAL);
	...
	/**
	 * Checks if the user has implicitly or explicitly declared standalone mode. This allows clear
	 * intent declaration to avoid warnings about missing network components.
	 * 
	 * @param rootConfig
	 *            the root configuration to check for standalone mode declaration
	 * @return true if standalone mode should be assumed (no warnings)
	 */
	private boolean isStandaloneModeIntended(Configuration rootConfig) {
		if (rootConfig != null) {
	...
	private static URI getListeningURI(MessageServer server) {
		if (server != null) {
			...
			LOGGER.debug("No message server configured - operating in standalone mode");
		}
		return null;
	}
}
