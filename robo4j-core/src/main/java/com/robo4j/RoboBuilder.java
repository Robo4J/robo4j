...
public final class RoboBuilder {
	...
	/**
	 * Configuration key for the maximum thread size for the scheduler thread pool.
	 */
	...
	/**
	 * Class responsible for the XML parsing. Using SAX to keep down resource requirements.
	 */
	 ...
	 *
	 * @param systemConfig
	 *            the configuration settings for the system. Note that this is separate from the
	 *            unit definitions today.
	...
	/**
	 * Instantiates a RoboUnit from the provided class using the provided id and adds it. The
	 * instance will be initialized using an empty configuration.
	 *
	 * @param clazz
	 *            the class to use.
	 * @param id
	...
	/**
	 * Returns the built {@link RoboContext}. This should be the final method called on the builder.
	 *
	...
	/**
	 * Returns the context being built. This should only be used so that units can be instantiated
	 * with more control outside the builder. Note that you should then add the instances to the
	 * builder through add.
	 *
	...
	/**
	 * Will load all the units from the definitions found in the XML file and add them to the
	 * builder.
	 *
	 * @param inputStream
	...
	private void internalAddUnit(RoboUnit<?> unit) throws RoboBuilderException {
		if (unit == null) {
			...
			throw new RoboBuilderException("Only one unit with the id " + unit.id() + " can be active at a time. Skipping " + unit.toString());
		}
		units.add(unit);
	}
	...
	private RoboUnit<?> instantiateAndInitialize(Class<? extends RoboUnit<?>> clazz, String id, Configuration configuration) throws RoboBuilderException {
