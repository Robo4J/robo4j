...
public class AdafruitLcdUnit extends I2CRoboUnit<LcdMessage> {
    ...
    public void onMessage(LcdMessage message) {
        try {
            ...
            LOGGER.error("Could not accept message:{}", message.toString(), e);
        }
    }
    ...
    private void processLcdMessage(LcdMessage message) throws IOException {
        switch (message.getType()) {
            ...
            case SCROLL:
                ...
                switch (message.getText().trim()) {
                    ...
                    case "right":
                        ...
                        break;
                    default:
                        LOGGER.error("Scroll direction {} is unknown", message.getText());
                        break;
                }
                break;
            case SET_TEXT:
            ...
            case STOP:
                ...
                break;
            default:
                LOGGER.error("demo not supported!:{}", message.getType());
                break;
        }
    }
